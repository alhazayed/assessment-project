...
### Why the two helper scripts exist (the important gotchas)
- **`scripts/dev-local-supabase.sh`** — the tracked migrations do **not** apply to a fresh database. The first 72 migrations (and the core assessment-content **seed** migrations) are 2-line stubs ("applied directly to remote"); the real schema is `supabase/migrations/20260619120000_schema_baseline.sql`. That baseline has a forward FK (`assessment_submissions` → `assessment_assignments` defined later) and the `20260627220000_admin_dashboard_materialized_views.sql` migration references non-existent columns (`profiles.user_type/full_name/email`) with SQL syntax errors. The helper runs Supabase from a throwaway **copy** with those blocks reordered/neutralised, and generates a `seed.sql` that restores the `auth.users` signup trigger (dropped on fresh rebuild by `20260619210813`). **The tracked `supabase/` files are left untouched.**
  - **Do NOT run `supabase start` / `supabase db reset` directly against the repo `supabase/` dir** — it fails on the forward FK, and `db reset` will wipe the running DB using the broken migrations. Always use the helper (`start` / `stop` / `status`).
