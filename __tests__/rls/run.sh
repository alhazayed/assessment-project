#!/usr/bin/env bash
# Boot an ephemeral Postgres, apply an RLS test file, report pass/fail.
# Requires: postgresql-16 binaries + a non-root user to own the cluster.
# Usage: bash __tests__/rls/run.sh [test-file.sql]
#   defaults to rls_isolation.test.sql; e.g. `run.sh assignments_idor.test.sql`
set -euo pipefail
...
SQL="$(cd "$(dirname "$0")" && pwd)/${1:-rls_isolation.test.sql}"
