...
export default function AdminAnalyticsPage() {
  ...
  if (!data) return (
    <div className="p-8 text-sm text-gray-400">Failed to load analytics data.</div>
  )
  ...
  const PeriodBadge = () => {
    ...
    return (
      <span className={`inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full ${flat ? 'bg-gray-100 text-gray-500' : up ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
        ...
        {up ? '+' : ''}{overallStats.periodChange}% vs prev 30d
      </span>
    )
  }
  ...
  const statCards = [
    {
      label: 'Total Submissions', value: overallStats.total.toLocaleString(),
      sub: `Mean: ${overallStats.avg} · Median: ${overallStats.median}`,
      icon: Brain, color: 'text-indigo-600 bg-indigo-50',
      extra: <PeriodBadge />,
    },
    {
      label: 'High-Risk Flags', value: overallStats.highRisk.toLocaleString(),
      sub: `${overallStats.highRiskPct}% of all submissions`,
      icon: AlertTriangle, color: 'text-red-600 bg-red-50', extra: null,
    },
    {
      label: 'Registered Users', value: overallStats.totalUsers.toLocaleString(),
      sub: `+${overallStats.newUsersThisMonth} new this month`,
      icon: Users, color: 'text-blue-600 bg-blue-50', extra: null,
    },
    {
      label: 'Score Std Dev', value: overallStats.stddev.toString(),
      sub: `Range: ${overallStats.min} – ${overallStats.max}`,
    ...
    <div className="p-8 max-w-7xl">
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">Platform-wide psychometric data analysis · {overallStats.total.toLocaleString()} submissions across {assessmentStats.length} assessments</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => exportData('detailed')} disabled={!!exporting}
            ...
            <Download className="w-4 h-4" />
            {exporting === 'detailed' ? 'Exporting…' : 'Full Export'}
          </button>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 mb-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-sm font-semibold text-gray-700">Submission Trends — Last 30 Days</h2>
          <div className="flex items-center gap-4 text-xs text-gray-500">
            <span className="flex items-center gap-1.5"><span className="w-3 h-0.5 bg-indigo-500 inline-block rounded" />Submissions</span>
            <span className="flex items-center gap-1.5"><span className="w-3 h-0.5 bg-red-400 inline-block rounded" />High Risk</span>
          </div>
        </div>
        <ResponsiveContainer width="100%" height={220}>
          <AreaChart data={dailySubmissions} margin={{ top: 5, right: 5, left: -15, bottom: 0 }}>
            ...
            <Area type="monotone" dataKey="submissions" name="Submissions" stroke="#6366f1" fill="url(#gSubs)" strokeWidth={2} dot={false} />
            <Area type="monotone" dataKey="highRisk" name="High Risk" stroke="#ef4444" fill="url(#gRisk)" strokeWidth={2} dot={false} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="grid grid-cols-5 gap-6 mb-6">
        <div className="col-span-2 bg-white border border-gray-200 rounded-xl p-6">
          <h2 className="text-sm font-semibold text-gray-700 mb-5">Severity Distribution (All Time)</h2>
        ...
        <div className="col-span-3 bg-white border border-gray-200 rounded-xl p-6">
          <h2 className="text-sm font-semibold text-gray-700 mb-5">Top Assessments by Volume</h2>
          {assessmentStats.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={240}>
              ...
              >
                ...
                <Tooltip content={<ChartTooltip />} />
                <Bar dataKey="count" name="Submissions" fill="#6366f1" radius={[0, 3, 3, 0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden mb-6">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          <div>
            <h2 className="text-sm font-semibold text-gray-700">Per-Assessment Statistical Analysis</h2>
            <p className="text-xs text-gray-400 mt-0.5">{assessmentStats.length} assessments with submissions · click headers to sort</p>
          </div>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                <th className="text-left px-4 py-3 font-medium text-gray-600 min-w-[180px]">Assessment</th>
                {tableCols.map(col => (
                  <th key={col.key}
                ...
                <th className="text-left px-4 py-3 font-medium text-gray-600 min-w-[140px]">Severity Mix</th>
              </tr>
            </thead>
                  ...
                  <tr key={a.id} className="hover:bg-gray-50/60">
                    <td className="px-4 py-3">
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">
                        {a.code}
                        {!a.is_active && <span className="ml-1 text-orange-500">· hidden</span>}
                      </p>
                    </td>
      ...
      <div className="grid grid-cols-3 gap-6">
        <div className="col-span-2 bg-white border border-gray-200 rounded-xl p-6">
          <h2 className="text-sm font-semibold text-gray-700 mb-5">New User Registrations — Last 30 Days</h2>
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={userGrowth} margin={{ top: 5, right: 5, left: -15, bottom: 0 }}>
              ...
              <YAxis tick={{ fontSize: 10, fill: '#9ca3af' }} tickLine={false} axisLine={false} allowDecimals={false} />
              <Tooltip content={<ChartTooltip />} />
              <Bar dataKey="count" name="New Users" fill="#14b8a6" radius={[3, 3, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>

        <div className="bg-white border border-gray-200 rounded-xl p-6">
          <h2 className="text-sm font-semibold text-gray-700 mb-5">User Role Breakdown</h2>
          <div className="space-y-3">
            {[
              { role: 'patient', label: 'Patients', color: 'bg-blue-500' },
              { role: 'clinician', label: 'Clinicians', color: 'bg-green-500' },
              { role: 'admin', label: 'Admins', color: 'bg-purple-500' },
              { role: 'superadmin', label: 'Super Admins', color: 'bg-red-500' },
          ...
          <div className="mt-6 pt-5 border-t border-gray-100">
            <p className="text-xs font-semibold text-gray-500 mb-3">Overall Population Stats</p>
            <div className="space-y-1.5">
              {[
                { label: 'Global Mean Score', value: overallStats.avg },
                { label: 'Global Median', value: overallStats.median },
                { label: 'Std Deviation', value: overallStats.stddev },
                { label: 'Score Range', value: `${overallStats.min}–${overallStats.max}` },
              ].map(({ label, value }) => (
