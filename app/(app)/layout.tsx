...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  if (user && profile) {
    return (
      <div className="flex min-h-screen bg-gray-50 dark:bg-[#0A1019]">
        ...
        <main id="main-content" className={`flex-1 ${lang === 'ar' ? 'mr-64' : 'ml-64'} overflow-auto bg-gray-50 dark:bg-[#0A1019]`}>{children}</main>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#0A1019]">
      ...
      <header className="bg-white dark:bg-[#0E1A26] border-b border-gray-200 dark:border-[#1E2C3A] sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-6 h-14 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2 font-semibold text-gray-900 dark:text-[#F4F8FB]">
