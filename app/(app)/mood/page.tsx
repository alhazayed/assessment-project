...
const TRIGGERS = ['stress', 'work', 'family', 'health', 'sleep', 'exercise', 'social', 'weather', 'diet']
    ...
    <div className="p-8 max-w-4xl">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">Track how you feel each day</p>
        </div>
      ...
      {showForm && (
        <div className="card p-6 mb-8">
          <h2 className="text-base font-semibold text-gray-900 mb-5">How are you feeling today?</h2>
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-3 gap-6">
              {[
                { key: 'mood_score', label: 'Mood', color: 'text-pink-600', emoji: '😊' },
                { key: 'energy_score', label: 'Energy', color: 'text-yellow-600', emoji: '⚡' },
                { key: 'anxiety_score', label: 'Anxiety', color: 'text-purple-600', emoji: '😰' },
            ...
            <div>
              <label className="label">Triggers (select all that apply)</label>
      ...
      {loading ? (
        ...
        <div className="card p-12 text-center">
          ...
          <h3 className="text-base font-medium text-gray-700 mb-2">No mood entries yet</h3>
          <p className="text-sm text-gray-400">Start tracking how you feel each day</p>
        </div>
      ) : (
                ...
                <div>
                  ...
                  {log.triggers.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-1">
                      {log.triggers.map(t => (
                        <span key={t} className="px-2 py-0.5 bg-gray-100 text-gray-500 text-xs rounded-full capitalize">{t}</span>
                      ))}
                    </div>
                  )}
                </div>
                <div className="text-right text-xs text-gray-400">
                  {log.sleep_hours && <p>Sleep: {log.sleep_hours}h</p>}
                  {log.activity_minutes && <p>Activity: {log.activity_minutes}min</p>}
                </div>
              </div>
